...
@test "$BATS_TEST_FILENAME :: Testing valid matrix with two entries" {
    ...
    [ "$(echo ${lines[-17]}|cut -d' ' -f6-)" == "Processing pipeline for matrix entry 'first'" ]
    [ "$(echo ${lines[-16]}|cut -d' ' -f6-)" == "Processing pipeline stage 'test'" ]
    [ "$(echo ${lines[-12]}|cut -d' ' -f6-)" == "| first hello world on matrix level" ]
